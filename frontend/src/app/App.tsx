...
const categories = [
  { id: 'all', label: 'Todo' },
  { id: 'video', label: 'Streaming', icon: Tv },
...
export default function App() {
  ...
  const filteredPlatforms = platforms.filter(p => activeFilter === 'all' || p.category === activeFilter);
    ...
    <div className="bg-[#09090b] text-white min-h-screen font-sans overflow-x-hidden selection:bg-rose-500/30">
      ...
      <header className="fixed top-0 w-full z-40 px-5 py-4 flex justify-between items-center bg-gradient-to-b from-[#09090b] via-[#09090b]/80 to-transparent pointer-events-none">
        <div className="flex items-center gap-2 pointer-events-auto">
          ...
          <span className="font-extrabold text-xl tracking-tight">Digital <span className="text-rose-500">Juanex</span></span>
        </div>
          ...
          <div className="relative z-10 max-w-lg mx-auto md:max-w-4xl">
            ...
            >
              ...
              <span className="text-rose-400 text-xs font-bold uppercase tracking-wider">
                Venta Relámpago Termina en: {String(timeLeft.h).padStart(2, '0')}:{String(timeLeft.m).padStart(2, '0')}:{String(timeLeft.s).padStart(2, '0')}
              </span>
            </motion.div>
            ...
            >
              Accede a Netflix, Disney+, Max y más desde <span className="text-white font-bold">$5.000/mes</span>. Entrega rápida y garantía total.
            </motion.p>
            ...
            >
              <div className="flex items-center gap-1.5 text-xs text-neutral-300">
                <CheckCircle2 className="w-4 h-4 text-green-500" /> Pantallas Personales
              </div>
              <div className="flex items-center gap-1.5 text-xs text-neutral-300">
                <span className="text-base leading-none">🇨🇴</span> Pesos Colombianos
              </div>
              <div className="flex items-center gap-1.5 text-xs text-neutral-300">
        ...
        <section className="w-full max-w-lg mx-auto md:max-w-4xl relative z-20">
          ...
          <div className="px-5">
            <AnimatePresence mode="popLayout">
              <div className="flex gap-4 overflow-x-auto pb-8 pt-2 snap-x snap-mandatory [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                {filteredPlatforms.map((platform, idx) => {
                  const inCart = cart.includes(platform.id);
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={cn(
                        ...
                        inCart ? "border-rose-500 shadow-[0_0_20px_rgba(244,63,94,0.2)]" : "border-neutral-800 bg-neutral-900/50"
                      )}>
                        {/* Background Gradient */}
                        <div className={cn(
                          "absolute inset-0 opacity-20 bg-gradient-to-br", platform.color
                        )} />
                        ...
                        <div className="relative z-10">
                          <h3 className="font-bold text-xl text-white mb-1">{platform.name}</h3>
                          ...
                          <div className="flex items-end gap-3 mb-5 p-3 rounded-2xl bg-black/40 border border-white/5">
                            <div>
                              <p className="text-[10px] text-neutral-500 uppercase font-bold tracking-wider mb-0.5">Precio Hoy</p>
                              <div className="flex items-baseline gap-2">
                                <span className="text-2xl font-black text-white">{platform.price}</span>
                                <span className="text-sm font-medium text-neutral-500 line-through">{platform.originalPrice}</span>
                              </div>
                            </div>
                          ...
                          {/* Action Button */}
                          <button 
                            onClick={() => toggleCart(platform.id)}
                            className={cn(
        ...
        <section className="px-5 mt-6 max-w-lg mx-auto md:max-w-4xl">
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-neutral-900/50 border border-neutral-800 rounded-2xl p-4 flex flex-col items-center text-center">
              ...
              <p className="text-[11px] text-neutral-400 leading-tight">Recibe tus credenciales por email y WhatsApp.</p>
            </div>
            <div className="bg-neutral-900/50 border border-neutral-800 rounded-2xl p-4 flex flex-col items-center text-center">
              <ShieldCheck className="w-8 h-8 text-emerald-400 mb-2" />
              <h4 className="font-bold text-sm mb-1">Garantía Total</h4>
              <p className="text-[11px] text-neutral-400 leading-tight">Soporte técnico 24/7 y garantía de reposición si algo falla.</p>
            </div>
          </div>
        </section>
              ...
              <button className="w-full bg-black/20 backdrop-blur-md rounded-xl p-4 flex items-center justify-between active:scale-[0.98] transition-transform">
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <p className="font-bold text-sm text-white leading-tight">Completar Compra</p>
                    <p className="text-xs text-rose-100/80">Estás ahorrando +50%</p>
                  </div>
                </div>
              ...
              >
                ...
                <Icon 
                  className={cn(
                    ...
                    isActive ? "text-rose-500 scale-110" : "text-neutral-500 group-hover:text-neutral-300"
                  )} 
                  strokeWidth={isActive ? 2.5 : 2}
                />
                <span className={cn(
                  "text-[10px] font-bold transition-colors duration-300",
                  isActive ? "text-rose-500" : "text-neutral-500 group-hover:text-neutral-300"
                )}>
                  {tab.label}
                </span>
              </button>
